<ul class="plain-nav-tabs nav nav-tabs js-enable-tab-url" id="component-1" role="tablist">
    <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#component-1-1" role="tab" aria-controls="component-1-1" aria-selected="true"><strong>My Products</strong></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-toggle="tab" href="#component-1-2" role="tab" aria-controls="component-1-2" aria-selected="false"><strong>All Products</strong></a>
    </li>
    <li class="nav-item newsearch">
        <a  href="#"><input type="search" id="#" name="gsearch" placeholder="  search products"></a>
    </li>
</ul>